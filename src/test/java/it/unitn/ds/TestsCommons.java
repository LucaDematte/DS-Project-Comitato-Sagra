...
public class TestsCommons {
    ...
    public static class TestsSystemWrapper {
        ...
        public TestsSystemWrapper(
                ActorSystem system, Map<Integer, ActorRef> actors,
                Map<Integer, TestKit> probes, int min_latency, int max_latency
        ) {
            this.system = system;
            this.actors = actors;
            this.probes = probes;
        ...
        public TestsSystemWrapper(
                ActorSystem system, Map<Integer, ActorRef> actors, Map<Integer, TestKit> probes) {
            this.system = system;
            this.actors = actors;
            this.probes = probes;
    ...
    public static TestsSystemWrapper createTestSystem(
            String name, int n_actors, int coordinator, int min_latency, int max_latency) {
        ...
        for (int i = 0; i < n_actors; i++) {
            ...
            probes.put(i, probe);
            group.put(i,
                      system.actorOf(Replica.propsWithListener(i,
                                                               min_latency,
                                                               max_latency,
                                                               TEST_COORDINATOR_BEAT_INTERVAL,
                                                               probe.getRef()
                                     ), "Replica_" + i
                      )
            );
