...
/**
 * The ask system enables an actor to send messages and execute an operation when a response is received or when
 * a specified timeout expires.
 * The behavior is similar to {@code Patterns.ask} (provided by Akka), but this implementation uses the custom
 * {@code tell} method (of {@link it.unitn.ds.AbstractReplica}) that all replicas in this project are required to use.
 * <p>
 * To make this work, the class {@link CSAskMessage} must be extended by messages that are sent with this system.
 * This class contains a {@link UUID} that is used to correlate a request with a response. Therefore, a response must
 * be sent with the same {@link UUID} as the request.
 * </p>
 * When asking a message, a callback can be registered to be executed when the response is received or when the
 * specified timeout expires.
 */
public class CSAsk {
    /** Context of the actor using the ask system. Used to schedule a reminder for the callback in case the timeout expires. */
    private final AbstractActor.ActorContext context;
    /** The underlying method that actors use when sending a normal message (for replicas it's the custom {@code tell}). */
    ...
    public interface Callback<T extends CSAskMessage> {
         ...
         * @param timedOut Flag set to {@code true} if the callback was called after the timeout expired.
         */
        void handle(T response, boolean timedOut);
    }
    
    /**
     * This method is the core functionality of this class: sends a request (ask), registers a callback
     * to be executed when the response is received or when the timeout expires.
     *
     * @param msg The message to be sent as request.
     * @param destination The actor that will receive the message.
     * @param timeout The timeout after which the callback is executed if a response is not received.
     * @param callback The callback to be executed when the response is received or when the timeout expires.
    ...
    /**
     * This method must be registered in the actor's receive bindings so that, when a response is received,
     * the ask system can execute the right callback.
    ...
    /**
     * This method must be registered in the actor's receive bindings to enable timeout expiration tracking.
     * A {@link CSAskTimeout} message is scheduled for every request, and when it is received before the response
    ...
    /**
     * An object representing a pending request. It stores the callback to be executed and the timer after which
     * the response is considered out of time.
     * @param <T> The type of the expected response.
     */
