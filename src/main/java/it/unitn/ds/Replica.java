...
public class Replica extends AbstractReplica {
     ...
     * The coordinator uses this timer to send a message to itself every second to remember to send heartbeat to other replicas.
     * A normal replica uses this timer to check every second if it has received {@link CSHeartBeatFromCoordinator} or not.
     */
    Cancellable heartBeatTimer;
    /** System that maintains information on how and when this replica should crash. More info at {@link CSCrashSystem}. */
    ...
    /**
     * Flag that signals if a heartbeat message has been received in the time frame of the last second.
     ...
     * If the flag is already {@code false}, this means that the coordinator didn't send a new heartbeat after the previous check.
     */
     ...
     * The coordinator must increment the value in this map whenever an ACK for an update is received.
     */
     ...
     * The queue is of {@link CSWriteForward} instead of {@link CSWriteRequest}
     * because additional information is needed and are inserted in {@link CSWriteForward}
     * to leave the implementation of {@link CSWriteRequest} clean.
     */
    ...
    /**
     * Handler for read requests coming from clients. A read request is resolved by immediately returning the value stored in this replica.
    ...
    /**
     * Handler for write requests coming from clients. A write request is resolved by forwarding it to the coordinator,
     * which will use the update protocol so that the update is applied to all replicas.
    ...
    /**
     * Special handler for write requests sent directly to the coordinator. The write request is added
     * to the queue and then processed with the update protocol so that the update is applied to all replicas.
    ...
    /**
     * Handler to receive requests forwarded to the coordinator. The update request is added to the queue
     * of the coordinator.
     * This handler is used only by the coordinator.
    ...
    /**
     * This method must be called whenever there are updates waiting to be processed in the queue of the coordinator.
     ...
     * A new UPDATE message ({@link CSUpdate}) with an update key is sent to all replicas so that they get to know the update data.
     * Inside this method it is also defined the behavior of the coordinator when it receives an ACK for the UPDATE message (inside a lambda).
    ...
    /**
     * This method writes an update to the {@code positions} array and uses the callback of the codebase.
    ...
    /**
     * This method checks if the replica should be the one in charge of sending a write result back to the client.
     * If this is the case, a write result ({@link CSWriteResult}) is sent to the client with the correct {@code askUUID} to mark it as a response to the client ask.
    ...
    /**
     * Handler used by normal replicas to receive the HEARTBEAT ({@link CSHeartBeatFromCoordinator}) from the coordinator. In this method,
     * the flag {@code heartBeatReceived} is set to true so that a following heartbeat check is successful.
     * This handler is used only by normal replicas.
     ...
     * @param msg The message being sent in multicast or broadcast. If the message is of a class that
     *            is a condition for crash ({@link CSUpdate}, {@link CSWriteOk}), the crash system is checked.
    ...
    /**
     * Handler used by normal replicas to update the {@code next} and {@code previous} references for the ring topology used during election.
     * This handler is used only by normal replicas.
     * @param msg A message sent by the coordinator that specifies the new previous and next replicas.
     */
     ...
     * @param handler The callback to execute when the response is received or when the timeout runs out.
     */
    public final void broadcast(Supplier<CSAskMessage> msgFactory, Duration timeout,
     ...
     * This is useful to prevent Akka from printing warnings for "dead letters" when a crashed replica
     * doesn't handle a message. By applying this handler to all messages, the output in console is cleaner of unwanted prints.
